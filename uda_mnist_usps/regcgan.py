...
def classifier(image, domain_y, batch_size, reuse=False):
  with tf.variable_scope('discriminator'): 
    tf.get_variable_scope().reuse_variables()

    domain_y_conv = tf.ones((batch_size, FLAGS.domain_num, FLAGS.output_size, 1))\
        *tf.reshape(domain_y, [batch_size, FLAGS.domain_num,1,1])
    x = tf.concat((domain_y_conv, image), axis=1)
    ...
    h1 = slim.flatten(h1)
    h2 = lrelu(linear(h1, 500, 'd_h2_lin'), name='d_h2_relu')
    
  with tf.variable_scope('classifier'): 
    if reuse:
      tf.get_variable_scope().reuse_variables()

    h3 = linear(h2, 10, 'd_cls_h3_lin')
...
def loss(D_logits_real, D_logits_fake, D_logits_fake_for_G, reg_g, reg_d, weight_G, weight_D, C_logits, labels_h1):
  ...
  labels = tf.cast(labels_h1, tf.int64)
  C_logits = C_logits[0:FLAGS.half_batch_size,:]

  cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
                  logits=C_logits, labels=labels, name='cross_entropy_per_example')
  C_loss = tf.reduce_mean(cross_entropy, name='cross_entropy')

  D_loss = D_loss_real + D_loss_fake + weight_D * reg_d + C_loss

  G_loss = G_loss + weight_G * reg_g
  
  return G_loss, D_loss

def loss_for_classifier(logits, labels):
...
def train_for_classifier(C_loss, C_vars, global_step):

  C_optim = tf.train.AdamOptimizer(FLAGS.learning_rate, beta1=FLAGS.beta1) 
  C_grads = C_optim.compute_gradients(C_loss, var_list=C_vars)
  C_train_op = C_optim.apply_gradients(C_grads, global_step=global_step)
...
def correct_num(logits, labels):
  ...
  num = tf.reduce_sum(tf.cast(correct_prediction, tf.float32))
