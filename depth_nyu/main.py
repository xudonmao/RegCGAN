...
tf.app.flags.DEFINE_integer('c_dim', '3', """y dim""")
...
tf.app.flags.DEFINE_string('data_path1', 'nyu_img_120_160.tfrecords', """Path to the lsun data file""")
tf.app.flags.DEFINE_string('data_path2', 'nyu_depth_120_160.tfrecords', """Path to the lsun data file""")
...
def CropImages(images1, images2, dim):
  _, H, W, _ = images1.shape
  x = random.randint(0, H - dim)
  y = random.randint(0, W - dim)
  return images1[:, x:x+dim,y:y+dim,:], images2[:, x:x+dim, y:y+dim,:]
...
def GenValsForHolder_TF(data_set, data_set2, sess):
  ...
  images_v1, images_v2 = CropImages(images_v1, images_v2, FLAGS.output_size)
